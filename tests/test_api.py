...
class Api(unittest.TestCase):
    ...
    def test_02_schedule_parse_and_ledger(self):
        ...
        self.assertEqual(r["count"], 2); self.assertEqual(r["weekly_hours"], 67.0)   # 保存课表即自动采用 70 − 课时
        w = self.c.get("/api/week").json()
        self.assertEqual(w["class_hours"], 3.0)                # 第 2 教学周两门课各 2 节
        self.assertEqual(w["gap"]["weekly_hours"], 67.0)
        ...
        self.assertEqual(m["week"]["gap"]["weekly_hours"], 67.0)
    ...
    def test_02b_semester_any_day_and_defaults(self):
        ...
        self.assertEqual(me["weekly_hours"], 70.0); self.assertEqual(me["semester_start"], "2026-09-13")
